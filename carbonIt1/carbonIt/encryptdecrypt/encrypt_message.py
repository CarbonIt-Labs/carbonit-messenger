"""
encrypt_message.py
====================
Encrypts outgoing messages using Fernet (AES-128 in CBC + HMAC)
and Carbon Shield multi-layer encoding for end-to-end encryption.

Part of: CarbonIt Secure Messenger
Author: Edwin Sam K Reju
License: MIT
"""

from cryptography.fernet import Fernet
import base64
import hashlib
from encryptdecrypt.shield_crypto import shield_encrypt


def pad_key(key: str) -> bytes:
    """
    Ensure the Fernet key is 32 url-safe base64 bytes.
    Deterministically expand shorter keys.
    """
    key_hash = hashlib.sha256(key.encode()).digest()
    return base64.urlsafe_b64encode(key_hash)


def encrypt_message(chat_key: str, message: str) -> str:
    """
    Encrypts message using Carbon Shield + Fernet symmetric key.
    (Shield first, then Fernet)
    """
    try:
        # Step 1: Shield layer (adds hash-salt obfuscation)
        shielded = shield_encrypt(message)

        # Step 2: Symmetric Fernet AES encryption
        fernet_key = pad_key(chat_key)
        fernet = Fernet(fernet_key)
        encrypted = fernet.encrypt(shielded.encode())

        return encrypted.decode()
    except Exception as e:
        print(f"[ENCRYPT ERROR] {e}")
        return ""
